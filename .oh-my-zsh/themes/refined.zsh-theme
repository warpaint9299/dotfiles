...
zstyle ':vcs_info:*:*' formats "$FX[bold]%r$FX[no-bold]/%S" "%F{#b8bb26}via %s:%b" "%%u%c"
...
precmd() {
    ...
    print -P "\n$(repo_information) %F{#92D100}$(cmd_exec_time)%f"
    unset cmd_timestamp #Reset cmd exec time.
}

# Define prompts
#
PROMPT="%(?.%F{#b8bb26}.%F{red})❯%f " # Display a red prompt char on failure
